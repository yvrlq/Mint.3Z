    ...
    public static class Functions
    {
        ...
        public static bool CorrectSlot(ItemType type, EquipmentSlot slot)
        {
            switch (slot)
            {
                ...
                case EquipmentSlot.SwChenghao:
                    return type == ItemType.SwChenghao;

                case EquipmentSlot.Shizhuang:
                    return type == ItemType.Shizhuang;
       
                case EquipmentSlot.Fabao:
                    return type == ItemType.Fabao;
                default:
                    return false;
            }
        }
        ...
        public static string ToString(TimeSpan time, bool details, bool small = false)
        {
            ...
            else if (time.TotalSeconds > 1 && time.Seconds > 0) textS = "不到一秒钟";
        ...
        public static MirDirection DirectionFromPoint_(Point source, Point dest)
        {
            if (source.X < dest.X)
            {
                if (source.Y < dest.Y)
                    return MirDirection.DownRight;
                return source.Y > dest.Y ? MirDirection.UpRight : MirDirection.Right;
            }
            if (source.X <= dest.X)
                return source.Y < dest.Y ? MirDirection.Down : MirDirection.Up;
            if (source.Y < dest.Y)
                return MirDirection.DownLeft;
            return source.Y > dest.Y ? MirDirection.UpLeft : MirDirection.Left;
        }
