    ...
    public sealed class EditCharacterDialog : DXWindow
    {
        ...
        public void OnChangeChanged(ChangeType oValue, ChangeType nValue)
        {
            ...
            switch (Change)
            {
                ...
                case ChangeType.NameChange:
                    ...
                    break;
                case ChangeType.ClassChange:
                    WarriorButton.Enabled = true;
                    WizardButton.Enabled = true;
                    TaoistButton.Enabled = true;
        ...
        public EditCharacterDialog()
        {
            ...
            CharacterNameHelpLabel = new DXLabel
            {
                ...
                Parent = this,
                Text = "[?]",
                Hint = $"人物名字.\n支持中文、字符和数字.\n长度: 在 {CartoonGlobals.MinCharacterNameLength} 和 {CartoonGlobals.MaxCharacterNameLength} 之间.\n同一个账户可以使用之前的名字.",
            };
        ...
        public void Confirm()
        {
            switch (Change)
            {
                ...
                case ChangeType.ClassChange:
                    ...
                    if (SelectedClass == GameScene.Game.User.Class)
                    {
                        GameScene.Game.ReceiveChat($"你已经是一个 {SelectedClass}.", MessageType.System);
                        return;
                    }
                    ...
                    CEnvir.Enqueue(new C.ClassChange { Class = SelectedClass });

                    if (MapObject.User.InSafeZone)
                    {
                        DXMessageBox box = new DXMessageBox($"你正在改职业中\n" +
                                      $"请等待 {(deleteTime - CEnvir.Now).TotalSeconds:0.0} 秒后游戏会小退", "小退游戏", DXMessageBoxButtons.YesNo);

                        box.YesButton.MouseClick += (o, e1) => CEnvir.Enqueue(new C.Logout());
                        box.YesButton.Enabled = false;
                        box.NoButton.Enabled = false;
                        box.CloseButton.Enabled = false;
                        ...
                        box.ProcessAction = () =>
                        {
                            if (CEnvir.Now > deleteTime)
                            {
                                ...
                                box.Label.Text = $"你正在改职业中\n" +
                                                 $"请等待 {(deleteTime - CEnvir.Now).TotalSeconds:0.0} 秒后游戏会小退";
                        };
                    }
                    break;
                default:
