    ...
    public sealed class SelectScene : DXScene
    {
        ...
        public sealed class SelectDialog : DXWindow
        {
            ...
            private void DeleteButton_MouseClick(object sender, MouseEventArgs e)
            {
                ...
                DXMessageBox box = new DXMessageBox($"是否确定要删除角色 {character.CharacterName}\n" +
                                                    $"请等待 {(deleteTime - CEnvir.Now).TotalSeconds:0.0} 确认前的秒数.", "删除角色", DXMessageBoxButtons.YesNo);
                ...
                box.ProcessAction = () =>
                {
                    ...
                    {
                        box.Label.Text = $"是否确定要删除角色 {character.CharacterName}.";
                        box.YesButton.Enabled = true;
                        box.ProcessAction = null;
                    }
                    else
                        box.Label.Text = $"是否确定要删除角色 {character.CharacterName}\n" +
                                         $"请等待 {(deleteTime - CEnvir.Now).TotalSeconds:0.0} 确认前的秒数.";
                };
            }
        ...
        public sealed class NewCharacterDialog : DXWindow
        {
            ...
            public void OnSelectedGenderChanged(MirGender oValue, MirGender nValue)
            {
                ...
                Type itemType = SelectedGender.GetType();
                MemberInfo[] infos = itemType.GetMember(SelectedGender.ToString());
            ...
            public NewCharacterDialog()
            {
                ...
                CharacterNameHelpLabel = new DXLabel
                {
                    ...
                    Parent = this,
                    Text = "[?]",
                    Hint = $"人物名字.\n支持中文、字符和数字.\n长度: 在 {CartoonGlobals.MinCharacterNameLength} and {CartoonGlobals.MaxCharacterNameLength} 之间.\n同一个账户可以使用之前的名字.",
                };
            ...
            {
                CharacterNameValid = CartoonGlobals.CharacterReg.IsMatch(CharacterNameTextBox.TextBox.Text);
        ...
        public sealed class SelectButton : DXControl
        {
            ...
            public void OnSelectInfoChanged(SelectInfo oValue, SelectInfo nValue)
            {
                ...
                ClassLabel.Text = description?.Description ?? SelectInfo.Class.ToString(); ;

                


                LevelLabel.Text = SelectInfo.Level.ToString();
                LocationLabel.Text = CartoonGlobals.MapInfoList.Binding.FirstOrDefault(x => x.Index == SelectInfo.Location)?.Description ?? "新角色";
            }
