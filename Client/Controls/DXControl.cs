...
namespace Client.Controls
{
    public class DXControl : IDisposable
    {
        ...
        public virtual void OnCanResizeHeightChanged(bool oValue, bool nValue)
        {
            ...
            EventHandler<EventArgs> resizeHeightChanged = CanResizeHeightChanged;
            if (resizeHeightChanged == null)
                return;
            resizeHeightChanged((object)this, EventArgs.Empty);
            */
        }
        ...
        public bool CanResizeHeightBottom
        {
            get
            {
                return _CanResizeHeightBottom;
            }
            set
            {
                if (_CanResizeHeightBottom == value)
                    return;
                bool resizeHeightBottom = _CanResizeHeightBottom;
                _CanResizeHeightBottom = value;
                OnCanResizeHeightChanged(resizeHeightBottom, value);
            }
        }
        ...
        {
            Rectangle b = Rectangle.Intersect(DXControl.ActiveScene.DisplayArea, displayArea);
            if (!control.IsMoving || !control.AllowDragOut)
            {
                for (; parent != null; parent = parent.Parent)
                {
                    Rectangle displayArea1;
                    if (parent.IsMoving && parent.AllowDragOut)
                    {
                        displayArea1 = DXControl.ActiveScene.DisplayArea;
                        b = Rectangle.Intersect(displayArea1, displayArea);
                        break;
                    }
                    displayArea1 = parent.DisplayArea;
                    b = Rectangle.Intersect(displayArea1, b);
                    if (!displayArea1.IntersectsWith(displayArea))
                        return;
                }
            }
            if (b.IsEmpty)
                return;
            b.Location = new Point(b.X - displayArea.X, b.Y - displayArea.Y);
            if (control.GetType() == typeof(DXImageControl))
            {
                DXImageControl dxImageControl = control as DXImageControl;
                DXManager.Device.SetSamplerState(0, SamplerState.AddressU, TextureAddress.Clamp);
                DXManager.Device.SetSamplerState(0, SamplerState.AddressV, TextureAddress.Clamp);
                if (dxImageControl != null && (uint)dxImageControl.TilingMode > 0U)
                {
                    if (dxImageControl.TilingMode == TilingMode.Horizontally || dxImageControl.TilingMode == TilingMode.All)
                        DXManager.Device.SetSamplerState(0, SamplerState.AddressU, TextureAddress.Wrap);
                    if (dxImageControl.TilingMode == TilingMode.Vertically || dxImageControl.TilingMode == TilingMode.All)
                        DXManager.Device.SetSamplerState(0, SamplerState.AddressV, TextureAddress.Wrap);
                }
                if (dxImageControl != null && dxImageControl.Zoom && (double)dxImageControl.Scale != 1.0)
                {
                    Sprite sprite1 = DXManager.Sprite;
                    int x1 = displayArea.X;
                    Point location = b.Location;
                    int x2 = location.X;
                    double num1 = (double)(x1 + x2 + offX);
                    int y1 = displayArea.Y;
                    location = b.Location;
                    int y2 = location.Y;
                    double num2 = (double)(y1 + y2 + offY);
                    Matrix matrix = Matrix.Transformation2D(new Vector2((float)num1, (float)num2), 0.0f, new Vector2(dxImageControl.Scale), new Vector2(0.0f), 0.0f, new Vector2(0.0f, 0.0f));
                    sprite1.Transform = matrix;
                    Sprite sprite2 = DXManager.Sprite;
                    Texture texture1 = texture;
                    Rectangle? sourceRect = new Rectangle?(new Rectangle(new Point(0), dxImageControl.Size));
                    Vector3? center = new Vector3?(Vector3.Zero);
                    int x3 = displayArea.X;
                    location = b.Location;
                    int x4 = location.X;
                    double num3 = (double)(x3 + x4 + offX);
                    int y3 = displayArea.Y;
                    location = b.Location;
                    int y4 = location.Y;
                    double num4 = (double)(y3 + y4 + offY);
                    double num5 = 0.0;
                    Vector3? position = new Vector3?(new Vector3((float)num3, (float)num4, (float)num5));
                    Color4 color = (Color4)colour;
                    sprite2.Draw(texture1, sourceRect, center, position, color);
                    DXManager.Sprite.Transform = Matrix.Transformation2D(new Vector2(0.0f, 0.0f), 0.0f, new Vector2(1f), new Vector2(0.0f), 0.0f, new Vector2(0.0f, 0.0f));
                    return;
                }
            }
            Sprite sprite3 = DXManager.Sprite;
            Texture texture2 = texture;
            Rectangle? sourceRect1 = new Rectangle?(b);
            Vector3? center1 = new Vector3?(Vector3.Zero);
            int x5 = displayArea.X;
            Point location1 = b.Location;
            int x6 = location1.X;
            double num6 = (double)(x5 + x6 + offX);
            int y5 = displayArea.Y;
            location1 = b.Location;
            int y6 = location1.Y;
            double num7 = (double)(y5 + y6 + offY);
            double num8 = 0.0;
            Vector3? position1 = new Vector3?(new Vector3((float)num6, (float)num7, (float)num8));
            Color4 color1 = (Color4)colour;
            sprite3.Draw(texture2, sourceRect1, center1, position1, color1);
            if (blend)
            {
                ...
                Sprite sprite1 = DXManager.Sprite;
                Texture texture1 = texture;
                Rectangle? sourceRect2 = new Rectangle?(b);
                Vector3? center2 = new Vector3?(Vector3.Zero);
                int x1 = displayArea.X;
                location1 = b.Location;
                int x2 = location1.X;
                double num1 = (double)(x1 + x2 + offX);
                ...
                double num2 = (double)(y1 + y2 + offY);
                double num3 = 0.0;
                Vector3? position2 = new Vector3?(new Vector3((float)num1, (float)num2, (float)num3));
                Color4 color2 = (Color4)colour;
                sprite1.Draw(texture1, sourceRect2, center2, position2, color2);
                DXManager.SetBlend(blending, blendRate);
            }
            else
            {
                Sprite sprite1 = DXManager.Sprite;
        ...
        public static void PresentTexture(Texture texture, DXControl parent, Rectangle displayArea, Color colour, DXControl control, Point SrcOffset, bool blend = false, float blendrate = 1f)
        {
            Rectangle b = Rectangle.Intersect(DXControl.ActiveScene.DisplayArea, displayArea);
            if (!control.IsMoving || !control.AllowDragOut)
            {
            ...
            if (b.IsEmpty)
                return;
            b.Location = new Point(b.X - displayArea.X + SrcOffset.X, b.Y - displayArea.Y + SrcOffset.Y);
            if (blend)
            {
                bool blending = DXManager.Blending;
                float blendRate = DXManager.BlendRate;
                DXManager.SetBlend(blend, blendrate);
                Sprite sprite = DXManager.Sprite;
                Texture texture1 = texture;
                Rectangle? sourceRect = new Rectangle?(b);
                Vector3? center = new Vector3?(Vector3.Zero);
                int x1 = displayArea.X;
                Point location = b.Location;
                int x2 = location.X;
                ...
                double num3 = 0.0;
                Vector3? position = new Vector3?(new Vector3((float)num1, (float)num2, (float)num3));
                Color4 color = (Color4)colour;
                sprite.Draw(texture1, sourceRect, center, position, color);
                DXManager.SetBlend(blending, blendRate);
            }
            else
            {
