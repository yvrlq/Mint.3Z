...
namespace Client.Envir
{
    public sealed class MirLibrary : IDisposable
    {
        ...
        public void DrawBlend(int index, float x, float y, Color4 colour, float sizex, float sizey, float rate, ImageType type, BlendState RendState = BlendState.None)
        {
            if (!CheckImage(index))
                return;
            MirImage image = Images[index];
            ...
            float x1 = 1f;
            float y1 = 1f;
            if ((double)sizex != 0.0 && (double)sizey != 0.0)
            {
                x1 = sizex / (float)image.Width;
                y1 = sizey / (float)image.Height;
            }
            DXManager.SetBlend(true, rate);
            DXManager.Sprite.Transform = Matrix.Scaling(x1, y1, 1f);
            DXManager.Sprite.Draw(texture, new Vector3?(Vector3.Zero), new Vector3?(new Vector3(x / x1, y / y1, 0.0f)), colour);
            ++CEnvir.DPSCounter;
