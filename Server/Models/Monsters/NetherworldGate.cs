...
namespace Server.Models.Monsters
{
    public class NetherworldGate : MonsterObject
    {
        ...
        protected override void OnSpawned()
        {
            ...
            DespawnTime = SEnvir.Now.AddMinutes(Config.MysteryShipOpen);

            foreach (SConnection con in SEnvir.Connections)
                con.ReceiveChat(string.Format(con.Language.NetherGateOpen, CurrentMap.Info.Description, CurrentLocation), MessageType.Notice);
        }
        ...
        public override void Process()
        {
            ...
            if (SEnvir.Now >= DespawnTime)
            {
                ...
                foreach (SConnection con in SEnvir.Connections)
                    con.ReceiveChat(con.Language.NetherGateClosed, MessageType.Notice);
                ...
                for (int i = CurrentMap.Objects.Count - 1; i >= 0; i--)
                {
                    ...
                    switch (ob.Race)
                    {
                        case ObjectType.Player:
                            ...
                            {
                                string fileName;
                                fileName = Path.Combine(SEnvir.NameListPath, Config.神舰记录名称 + ".txt");
