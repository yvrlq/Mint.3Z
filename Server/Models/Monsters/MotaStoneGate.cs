...
{
    public class MotaStoneGate :MonsterObject
    {
        ...
        protected override void OnSpawned()
        {
            ...
            DespawnTime = SEnvir.Now.AddMinutes(Config.MotaOpen);

            foreach (SConnection con in SEnvir.Connections)
                con.ReceiveChat(string.Format(con.Language.MotaOpen, CurrentMap.Info.Description, CurrentLocation), MessageType.System);
        ...
        public override void Process()
        {
            ...
            if (SEnvir.Now >= DespawnTime)
            {
                ...
                foreach (SConnection con in SEnvir.Connections)
                     con.ReceiveChat(con.Language.MotaClosed, MessageType.System);
            ...
            if (SEnvir.Now >= SearchTime && SEnvir.MotaMapRegion != null && SEnvir.MotaMapRegion.PointList.Count > 0)
            {
                SearchTime = SEnvir.Now.AddSeconds(3);
                Map map = SEnvir.GetMap(SEnvir.MotaMapRegion.Map);
                ...
                for (int i = CurrentMap.Objects.Count - 1; i >= 0; i--)
                {
                    ...
                    switch (ob.Race)
                    {
                        case ObjectType.Player:
                            ...
                            ob.Teleport(map, SEnvir.MotaMapRegion.PointList[SEnvir.Random.Next(SEnvir.MotaMapRegion.PointList.Count)]);
                        
                            if (Config.魔虫洞是否记录)
                            {
                                string fileName;
                                fileName = Path.Combine(SEnvir.NameListPath, Config.魔虫洞记录名称 + ".txt");
