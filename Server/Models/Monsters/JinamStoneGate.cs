...
namespace Server.Models.Monsters
{
    public class JinamStoneGate :MonsterObject
    {
        ...
        protected override void OnSpawned()
        {
            ...
            DespawnTime = SEnvir.Now.AddMinutes(Config.LairRegionOpen);

            foreach (SConnection con in SEnvir.Connections)
                con.ReceiveChat(string.Format(con.Language.LairGateOpen, CurrentMap.Info.Description, CurrentLocation), MessageType.Notice);
        ...
        public override void Process()
        {
            ...
            if (SEnvir.Now >= DespawnTime)
            {
                ...
                foreach (SConnection con in SEnvir.Connections)
                    con.ReceiveChat(con.Language.LairGateClosed, MessageType.Notice);
                ...
                for (int i = CurrentMap.Objects.Count - 1; i >= 0; i--)
                {
                    ...
                    switch (ob.Race)
                    {
                        case ObjectType.Player:
                            ...
                            /*
                            string fileName;
                            fileName = Path.Combine(SEnvir.NameListPath, Config.熔岩记录名称 + ".txt");
